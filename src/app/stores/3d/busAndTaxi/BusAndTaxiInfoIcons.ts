...
import { TaxiWithinTripsZone } from './BusAndTaxiDataTypes';
...
export default class BusAndTaxiInfoIcons
  extends Group
  implements IDataModelDisplayGroup<string, TaxiWithinTripsZone, TaxiWithinTripsZone>
{
  ...
  private map = new Map<string, TaxiWithinTripsZone>();
  ...
  public create(fenceMap: Map<string, TaxiWithinTripsZone>): void {
    this.map = fenceMap;
  }
  ...
  private isNonRecurent(param: TaxiWithinTripsZone): boolean {
    return param.peopleCountParamsData.recurrency === 2;
  }

  public setData(
    current: Map<string, TaxiWithinTripsZone>,
    _target: Map<string, TaxiWithinTripsZone>,
    ...
    for (const [key, param] of current) {
      ...
      if (fence && this.isNonRecurent(param)) {
        ...
        popupPoint.z = (param.peopleCountParamsData.peopleCount / 1000) * 100;
        if (popupPoint) {
